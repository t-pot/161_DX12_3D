...
namespace tpot
{
	...
	private:
		...
		winrt::com_ptr<ID3D12Resource> 深度ステンシル_;// ■追加
		winrt::com_ptr<ID3D12DescriptorHeap> rtvヒープ_ = nullptr;
		winrt::com_ptr<ID3D12DescriptorHeap> dsvヒープ_ = nullptr;// ■追加
		winrt::com_ptr<ID3D12DescriptorHeap> オブジェクトヒープ_ = nullptr;// ■修正
		UINT rtv記述子サイズ_ = 0;
		...
		winrt::com_ptr<ID3D12Resource> テクスチャ_;
		winrt::com_ptr<ID3D12Resource> 定数バッファ_;// ■追加
		オブジェクト定数バッファ *マップされたオブジェクト定数バッファ_;// ■追加
